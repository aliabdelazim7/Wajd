...
const rawApi = {
  ...
  storage: {
    async uploadImage(bucket, file, folder = 'uploads') {
      ...
      // Validation: Size limits (max 5MB to match Laravel backend config)
      ...
      if (!['image/jpeg', 'image/png', 'image/webp', 'image/svg+xml', 'image/gif'].includes(file.type)) {
        throw new Error('صيغة الملف غير مدعومة. يرجى رفع صور JPG, PNG, WEBP, GIF or SVG');
      }
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || 'حدث خطأ أثناء رفع الصورة');
      }

      const data = await res.json();
      rawApi.auditLogs.log('رفع ملف', `تم رفع صورة جديدة إلى السيرفر: ${data.url}`);
      return data.url;
    },
  ...
  portfolio: {
    ...
    async upsert(project) {
      ...
      const data = await res.json();
      rawApi.auditLogs.log('تحديث معرض الأعمال', `إضافة أو تعديل دراسة حالة: ${data.name}`);
      return data;
    },
    async delete(id) {
  ...
  leads: {
    async getAll(statusFilter = 'all') {
      ...
      const data = await res.json();
      if (statusFilter !== 'all') {
        return data.filter(lead => lead.status.toLowerCase() === statusFilter.toLowerCase());
      }
      return data;
    },
