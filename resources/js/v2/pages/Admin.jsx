...
const StatCard = ({ label, value, icon: Icon, accent = false }) => (
    <div className={`rounded-2xl border p-5 ${accent ? 'border-gold-500/30 bg-gold-500/[0.08]' : 'border-white/10 bg-white/[0.035]'}`}>
        ...
        <strong className="font-serif text-4xl text-white">{value ?? 0}</strong>
    </div>
);
...
const Admin = () => {
    ...
    const records = useMemo(() => {
        if (active === 'leads' || active === 'audit') return payload?.data || [];
    ...
    const openEditor = (record = null) => {
        if (active === 'packages') setEditor(record ? { ...record, features_ar: (record.features_ar || []).join('\n'), features_en: (record.features_en || []).join('\n') } : { ...emptyPackage });
...
const Overview = ({ payload }) => {
    ...
    return <div className="space-y-8"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="كل العملاء المحتملين" value={counts.leads} icon={Users} /><StatCard label="طلبات جديدة" value={counts.new_leads} icon={BarChart3} accent /><StatCard label="الباقات المنشورة" value={counts.packages} icon={PackageOpen} /><StatCard label="دراسات الحالة" value={counts.projects} icon={BriefcaseBusiness} /><StatCard label="الأسئلة الشائعة" value={counts.faqs} icon={HelpCircle} /></div><div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]"><div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]"><div className="flex items-center justify-between border-b border-white/10 p-5"><h2 className="font-serif text-xl">آخر الطلبات</h2><span className="text-xs text-white/35">آخر 8 طلبات</span></div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-right text-sm"><thead className="text-white/35"><tr><th className="p-4">الاسم</th><th className="p-4">الخدمة</th><th className="p-4">الميزانية</th><th className="p-4">الحالة</th><th className="p-4">التاريخ</th></tr></thead><tbody>{(payload?.recent_leads || []).map((lead) => <tr key={lead.id} className="border-t border-white/5"><td className="p-4">{lead.name}<br /><span className="text-xs text-white/35">{lead.email}</span></td><td className="p-4 text-white/60">{lead.service || '—'}</td><td className="p-4 text-gold-500">{lead.budget_sar ? `${lead.budget_sar.toLocaleString()} SAR` : '—'}</td><td className="p-4"><span className="rounded-full bg-white/10 px-3 py-1 text-xs">{statusLabels[lead.status] || lead.status}</span></td><td className="p-4 text-white/35">{new Date(lead.created_at).toLocaleDateString('ar-SA')}</td></tr>)}{!payload?.recent_leads?.length && <tr><td colSpan="5" className="p-10 text-center text-white/35">لا توجد طلبات بعد.</td></tr>}</tbody></table></div></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><h2 className="mb-5 font-serif text-xl">آخر النشاطات</h2><div className="space-y-4">{(payload?.recent_activity || []).map((activity) => <div key={activity.id} className="flex items-start gap-3 border-b border-white/5 pb-4"><div className="mt-1 h-2 w-2 rounded-full bg-gold-500" /><div><p className="text-sm text-white/75">{activity.action} · {activity.entity}</p><p className="mt-1 text-xs text-white/35">{activity.user?.name || 'System'} · {new Date(activity.created_at).toLocaleString('ar-SA')}</p></div></div>)}{!payload?.recent_activity?.length && <p className="text-sm text-white/35">سيظهر النشاط الإداري هنا.</p>}</div></div></div></div>;
};
...
const LeadsSection = ({ records, search, setSearch, updateLeadStatus, remove }) => <div className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="font-serif text-3xl">العملاء المحتملون</h2><p className="mt-2 text-sm text-white/40">تابع كل طلبات تدقيق النمو من الموقع وحدّث مراحل المتابعة.</p></div><div className="relative"><Search className="absolute right-3 top-3 h-4 w-4 text-white/30" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث بالاسم أو البريد" className={`${inputClass} w-72 pr-10`} /></div></div><div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.03]"><table className="w-full min-w-[1000px] text-right text-sm"><thead className="text-white/35"><tr><th className="p-4">العميل</th><th className="p-4">التواصل</th><th className="p-4">الخدمة</th><th className="p-4">الميزانية</th><th className="p-4">الحالة</th><th className="p-4">التاريخ</th><th className="p-4">إجراء</th></tr></thead><tbody>{records.filter((lead) => `${lead.name} ${lead.email || ''} ${lead.phone || ''}`.toLowerCase().includes(search.toLowerCase())).map((lead) => <tr key={lead.id} className="border-t border-white/5 align-top"><td className="p-4"><strong>{lead.name}</strong>{lead.message && <p className="mt-2 max-w-xs text-xs leading-5 text-white/35">{lead.message}</p>}</td><td className="p-4 text-white/60">{lead.email}<br />{lead.phone || '—'}</td><td className="p-4 text-white/55">{lead.service || 'Growth Audit'}</td><td className="p-4 text-gold-500">{lead.budget_sar ? `${lead.budget_sar.toLocaleString()} SAR` : '—'}</td><td className="p-4"><select value={lead.status} onChange={(event) => updateLeadStatus(lead.id, event.target.value)} className="rounded-lg border border-white/10 bg-[#121212] px-3 py-2 text-xs text-white"><option value="new">جديد</option><option value="contacted">تم التواصل</option><option value="qualified">مؤهل</option><option value="closed">مغلق</option><option value="lost">غير مهتم</option></select></td><td className="p-4 text-white/35">{new Date(lead.created_at).toLocaleDateString('ar-SA')}</td><td className="p-4"><Button variant="danger" onClick={() => remove(lead.id, 'leads')}><Trash2 className="h-4 w-4" />حذف</Button></td></tr>)}{!records.length && <tr><td colSpan="7" className="p-12 text-center text-white/35">لا توجد طلبات مطابقة.</td></tr>}</tbody></table></div></div>;
