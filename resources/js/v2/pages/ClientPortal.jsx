...
const ClientPortal = () => {
    ...
    const copy = isArabic ? {
        ...
        returnLabel: 'العائد المتوقع',
        budgetLabel: 'الميزانية',
        customProject: 'مشروع مخصص',
        taskUnit: 'مهام',
                    ...
                    <div className="mx-auto max-w-7xl">
                        ...
                        <div className="grid min-h-[660px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#121210] shadow-2xl shadow-black/30 lg:grid-cols-[220px_minmax(0,1fr)]">
                            ...
                            <div className="min-w-0 p-5 md:p-8">
                                <div className={`mb-7 flex flex-wrap items-start justify-between gap-5 ${isArabic ? 'text-right' : 'text-left'}`}><div><p className="text-xs uppercase tracking-[0.2em] text-gold-500">{loading ? (isArabic ? 'جاري تحميل بيانات المشروع...' : 'Loading project snapshot...') : (isArabic ? 'آخر تحديث متصل بقاعدة البيانات' : 'Connected to live CMS')}</p><h2 className="mt-2 font-serif text-3xl text-white md:text-4xl">{snapshot ? `${isArabic ? 'مرحباً،' : 'Welcome,'} ${snapshot.client_name}` : (isArabic ? 'صباح الخير، علي' : 'Good morning, Ali')}</h2></div><button type="button" onClick={() => trackAnalyticsEvent('portal_message_clicked', { location: 'portal_preview' })} className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm text-white/60 transition hover:border-gold-500/40 hover:text-gold-500"><MessageCircle className="h-4 w-4" /> {isArabic ? 'أرسل تحديثاً' : 'Send an update'}</button></div>
                                <div className="grid gap-3 md:grid-cols-3">
                                    <KpiCard icon={ShieldCheck} label={copy.health} value={snapshot?.health || copy.healthy} accent />
                                    <KpiCard icon={Target} label={snapshot ? copy.returnLabel : copy.next} value={snapshot ? `${snapshot.estimated_monthly_return_sar.toLocaleString()} SAR` : copy.nextValue} />
                                    <KpiCard icon={Clock3} label={snapshot ? copy.budgetLabel : copy.openItems} value={snapshot ? `${snapshot.budget_sar.toLocaleString()} SAR` : '04'} />
                                </div>
