                        ...
                        <div className="grid min-h-[660px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#121210] shadow-2xl shadow-black/30 lg:grid-cols-[220px_minmax(0,1fr)]">
                            ...
                            <div className="min-w-0 p-5 md:p-8">
                                ...
                                <div className="grid gap-3 md:grid-cols-3">
                                    ...
                                    <KpiCard icon={Target} label={snapshot ? (isArabic ? 'العائد المتوقع' : 'Est. Return') : copy.next} value={snapshot ? `${snapshot.estimated_monthly_return_sar.toLocaleString()} SAR` : copy.nextValue} />
                                    <KpiCard icon={Clock3} label={snapshot ? (isArabic ? 'الميزانية' : 'Budget') : copy.openItems} value={snapshot ? `${snapshot.budget_sar.toLocaleString()} SAR` : '04'} />
                                </div>
                                ...
                                <div className="pt-7">
                                    {activeTab === 'timeline' && <div><div className={`mb-5 flex items-center justify-between gap-4 ${isArabic ? 'text-right' : 'text-left'}`}><h3 className="font-serif text-2xl text-white">{copy.timelineTitle}</h3><span className="text-xs text-white/25">{snapshot ? (isArabic ? 'مشروع مخصص' : 'Custom Project') : `4 / 6 ${isArabic ? 'مهام' : 'tasks'}`}</span></div><div className="space-y-3">{(snapshot?.project_stages || copy.timeline).map((stage, index) => {
