...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${scrolled ? 'py-3 lg:py-4 bg-obsidian-950/90 backdrop-blur-xl border-b border-white/5' : 'py-5 lg:py-6 bg-transparent'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div 
                        ...
                        className="fixed inset-0 z-[90] bg-obsidian-950/98 backdrop-blur-2xl flex flex-col justify-center items-center gap-10 md:hidden p-10 text-center"
                    >
                        {navLinks.map((link) => (
                            <Link 
                                key={link.path} 
                                ...
                                className="text-4xl font-serif hover:text-gold-500 transition-colors"
                            >
                                {link.name}
                            </Link>
                        ))}
                        <Link 
                            to="/contact" 
                            onClick={() => setIsOpen(false)}
                            className="mt-8 bg-gold-500 text-obsidian-950 px-12 py-5 rounded-full text-2xl font-arabic font-bold shadow-[0_0_30px_rgba(197,168,98,0.3)]"
                        >
                            {t.nav.cta}
                        </Link>
