...
export const mergePortfolioProjects = (cmsProjects, lang = 'ar') => {
    const evidenceProjects = Object.keys(evidence).map((slug) => getEvidenceProject(slug, lang));
    const merged = [...evidenceProjects];
    cmsProjects.forEach((cms) => {
        if (!merged.find((e) => e.slug === cms.slug)) {
            merged.push(cms);
        }
    });
    return merged;
